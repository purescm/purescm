...
(library (purs runtime code-unit-vector)
  ...
  ; * `bv`: a bytevector or #f
  ; * `pointer`: ftype-pointer or #f
  ; * `length`: length in code units
  (define-structure
    (code-unit-vector bv length))
  ...
  (define empty-code-unit-vector
    (make-code-unit-vector (make-immobile-bytevector 0) 0))

  (define (bytevector->code-unit-vector bv)
    (make-code-unit-vector bv (fx/ (bytevector-length bv) code-unit-length)))

  (define (code-unit-vector-alloc n)
    (make-code-unit-vector (make-immobile-bytevector (fx* n code-unit-length)) n))
  ...
  (define (code-unit-vector-&ref vec i)
    (ftype-&ref unsigned-16
                ()
                (make-ftype-pointer unsigned-16 (object->reference-address (code-unit-vector-bv vec)))
                i))
